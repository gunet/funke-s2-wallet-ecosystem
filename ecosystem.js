...
if (action === "down") {
	...
	execSync(`COMPOSE_PROFILES=* ${dockerComposeCommand} down`, { stdio: 'inherit' });
	process.exit();
}
...
function init() {
	...
	const firstIssuerInsertion = `INSERT INTO credential_issuer (credentialIssuerIdentifier, clientId, visible) VALUES ('https://demo.pid-issuer.bundesdruckerei.de/c', '1233', 1)`;

	const secondIssuerInsertion = `INSERT INTO credential_issuer (credentialIssuerIdentifier, clientId, visible) VALUES ('http://wallet-enterprise-por-issuer:8000', '1233', 1)`;
...
function buildImages() {
	...
	if (args.length <= 1 || args.includes("wallet-frontend")) {
		execSync(`cd wallet-frontend && docker build -t ghcr.io/gunet/wallet-frontend:${imageTag} .`, { stdio: 'inherit' });
	}

	if (args.length <= 1 || args.includes("wallet-backend-server")) {
		execSync(`cd wallet-backend-server && docker build -t ghcr.io/gunet/wallet-backend-server:${imageTag} .`, { stdio: 'inherit' });
	}
	...
	if (args.length <= 1 || args.includes("acme-verifier")) {
		execSync(`cd wallet-enterprise && docker build -t ghcr.io/gunet/wallet-enterprise:base -f base.Dockerfile .`, { stdio: 'inherit' });
		execSync(`docker build -t ghcr.io/gunet/wallet-enterprise-acme-verifier:${imageTag} -f wallet-enterprise-configurations/acme-verifier/Dockerfile .`, { stdio: 'inherit' });
	}
}
...
if (daemonMode === false) {
	...
	execSync(`${process.env.COMPOSE_PROFILES ? `COMPOSE_PROFILES=${process.env.COMPOSE_PROFILES}` : ''} ${dockerComposeCommand} up --build`, { stdio: 'inherit' });
} else {
	console.log("Performing 'docker compose up -d'");
	execSync(`${process.env.COMPOSE_PROFILES ? `COMPOSE_PROFILES=${process.env.COMPOSE_PROFILES}` : ''} ${dockerComposeCommand} up --build -d`, { stdio: 'inherit' });
}
